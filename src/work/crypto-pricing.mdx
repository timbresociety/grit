...
| --------------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Multi-source with methodology-aware selection | Single authoritative source        | No single source covers all assets reliably; different use cases require different methodologies               |
| Preserve source metadata with every price     | Store only "final" price           | Audit requires traceability; debugging requires knowing which source was used                                  |
| Timestamp alignment layer                     | Trust provider timestamps directly | Providers have clock drift and different aggregation windows; cross-source queries need consistent time domain |
| Immutable price snapshots at ingestion        | Re-fetch historical data as needed | Providers sometimes revise historical data; audit requires knowing what was known at decision time             |
| Configurable methodology per organization     | One-size-fits-all pricing          | Different accounting standards (GAAP, IFRS) and use cases require different approaches                         |
...
For how pricing data integrates with blockchain data indexing:
- **[[Multi-Chain Blockchain Data Indexing and Analytics Infrastructure]]**

For the broader system architecture:
